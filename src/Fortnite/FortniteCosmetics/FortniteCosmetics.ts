...
export class FortniteCosmetics {
    ...
    public respondWithNewCosmetics(i: AutocompleteInteraction<CacheType>): void | Promise<void> {
        ...
        const newItems = [...this._data].sort((a, b) => {
            ...
            return priorities[a.type.value] - priorities[b.type.value];
    ...
    private async replyEmbed(i: BaseCommandInteraction<CacheType>): Promise<void> {
        ...
        cosmetic.gameplayTags?.join().includes("BattlePass.Paid") && e.addField("Battle Pass", cosmetic.introduction ? `Obtained in the Chapter paid ${cosmetic.introduction?.chapter}, Season ${cosmetic.introduction?.season} Battle Pass.` : `Obtained in the paid Battle Pass.`)

        cosmetic.gameplayTags?.join().includes("Emote.Traversal") && e.addField("Emote Type", "Traversal")
        cosmetic.gameplayTags?.join().includes("BuiltIn") && e.addField("Emote Type", "Built In")
        cosmetic.gameplayTags?.join().includes("Cosmetics.UserFacingFlags.Synced") && e.addField("Emote Type", "Synced")

        // e.setColor("#2186DB")
        //@ts-ignore

        e.setColor(`#${cosmetic.series?.colors[1].slice(0, -2) || "FFFFFF"}`)
