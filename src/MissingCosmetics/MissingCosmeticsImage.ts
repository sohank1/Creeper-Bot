...
function buildHtml(items: MissingCosmeticImageItem[], shopDateLabel: string): string {
...
<footer class="footer"><span>Previously gone for <b>300+ days</b></span><span>Shop rotation · ${escapeHtml(shopDateLabel)}</span></footer>
</body></html>`;
}

function buildLockerFilesHtml(items: MissingCosmeticImageItem[], shopDateLabel: string): string {
...
</style></head><body class="${bodyClass}"><header><div class="stamp">RETURN</div><div class="small">DAILY ITEM SHOP ARCHIVE<b>LOCKER FILES</b></div><div class="date">ROTATION DATE<b>${escapeHtml(shopDateLabel)}</b></div></header><main>${cards}</main><footer><span>File status · <b>reissued</b></span><span>${items.length} cosmetic${items.length === 1 ? "" : "s"} absent 300+ days</span></footer></body></html>`;
}

function buildStormScanHtml(items: MissingCosmeticImageItem[], shopDateLabel: string): string {
...
</style></head><body class="${bodyClass}"><header><div class="brand">LOOP MONITOR // ITEM SHOP<b>STORM SCAN</b></div><div class="status">${items.length} SIGNAL${items.length === 1 ? "" : "S"} FOUND</div></header><main>${cards}</main><footer><span>Scan threshold <b>300+ days</b></span><span>Rotation ${escapeHtml(shopDateLabel)}</span></footer></body></html>`;
}

function buildReturnPassHtml(items: MissingCosmeticImageItem[], shopDateLabel: string): string {
    const cards = items.map((item, index) => `
...
</style></head><body><header><div><div class="pass-kicker">DAILY SHOP RE-ENTRY</div><div class="pass-title">THE RETURN PASS</div></div><div class="pass-summary"><span>PASSES ISSUED<b>${items.length}</b></span><span>LONGEST WAIT<b>${longest.toLocaleString("en-US")} DAYS</b></span></div></header><main>${cards}</main><footer><span>Admit to item shop · <b>${escapeHtml(shopDateLabel)}</b></span><span>Verified absence · <b>300+ days</b></span></footer></body></html>`;
}

function buildBusArrivalsHtml(items: MissingCosmeticImageItem[], shopDateLabel: string): string {
...
function buildItemShopHtml(items: MissingCosmeticImageItem[], shopDateLabel: string): string {
...
</style></head><body><nav class="game-nav"><span>PLAY</span><span>BATTLE PASS</span><span>COMPETE</span><span>LOCKER</span><b>ITEM SHOP</b><span>CAREER</span><span>V-BUCKS</span></nav><header class="section-head"><h1>RETURNING <span>◷ ${items.length} ITEM${items.length === 1 ? "" : "S"}</span></h1><p>SHOP ROTATION · ${escapeHtml(shopDateLabel)}</p></header><main class="offers">${tiles}</main><footer class="shop-footer"><span>Missing for <b>300+ days</b></span><span>All returning cosmetics</span></footer></body></html>`;
}

function buildLegacyFortniteItemShopReplicaHtml(items: MissingCosmeticImageItem[], shopDateLabel: string): string {
...
function buildFortniteItemShopReplicaHtml(items: MissingCosmeticImageItem[], shopDateLabel: string): string {
...
.fs-section{margin:0 0 30px 22px}
.fs-report-footer{margin:24px 0 0 22px;color:#a9dcff;font-size:10px;letter-spacing:.5px}
</style></head><body class="${useFeaturedMosaic ? "fs-mosaic" : "fs-dense"}"><header class="fs-section"><h1>BACK FROM THE VAULT</h1></header><main class="fs-grid">${tiles}</main><footer class="fs-report-footer">SHOP ${escapeHtml(shopDateLabel)} · ${items.length} RETURNING ITEMS · 300+ DAYS AWAY</footer><script>window.fitFortniteText=()=>{document.querySelectorAll(".fs-fit").forEach(node=>{const element=node;const minimum=Number(element.dataset.minSize||7);let size=parseFloat(getComputedStyle(element).fontSize);while(element.scrollWidth>element.clientWidth&&size>minimum){size=Math.max(minimum,size-.5);element.style.fontSize=size+"px"}})};document.fonts.ready.then(window.fitFortniteText);</script></body></html>`;
}

function buildSupplyDropHtml(items: MissingCosmeticImageItem[], shopDateLabel: string): string {
...
export async function renderMissingCosmeticsImage(
    ...
    try {
        ...
        await page.setViewport({ width: 1440, height: variant === "item-shop" ? 780 : 810, deviceScaleFactor: 1 });
        const html = variant === "locker-files"
            ? buildLockerFilesHtml(items, shopDateLabel)
            : variant === "storm-scan"
                ? buildStormScanHtml(items, shopDateLabel)
                : variant === "return-pass"
                    ? buildReturnPassHtml(items, shopDateLabel)
                    : variant === "bus-arrivals"
                        ? buildBusArrivalsHtml(items, shopDateLabel)
                        : variant === "item-shop"
                            ? buildFortniteItemShopReplicaHtml(items, shopDateLabel)
                        : variant === "supply-drop"
                            ? buildSupplyDropHtml(items, shopDateLabel)
                            : variant === "island-broadcast"
                                ? buildIslandBroadcastHtml(items, shopDateLabel)
                : buildHtml(items, shopDateLabel);
