...
export interface ShopImageLabels { title: string; subtitle: string; footer: string; profileName?: string; profileAvatar?: string; pageCount?: number }
...
export function buildFortniteItemShopReplicaHtml(items: MissingCosmeticImageItem[], shopDateLabel: string, minimumDays = 300, logoUrl?: string, labels?: ShopImageLabels): string {
    ...
    const tiles = displayItems.map((item, index) => {
        ...
        const statusPath = item.badgeLabel === "PAUSED" ? '<path d="M8 5v14M16 5v14"/>'
            : item.badgeLabel === "EVERY RETURN" ? '<path d="M17 2l4 4-4 4M3 11V9a3 3 0 0 1 3-3h15M7 22l-4-4 4-4m14-1v2a3 3 0 0 1-3 3H3"/>'
            : '<path d="M18 8a6 6 0 0 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9M10 21h4"/>';
        const statusIcon = item.badgeLabel ? `<svg class="fs-status-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${statusPath}</svg>` : "";
...
.fs-status-icon{display:inline-block;width:1em;height:1em;vertical-align:-.1em;margin-right:5px;overflow:visible}
...
${labels ? `body.fs-dense{width:${Math.max(720, Math.min(1440, columns * (tileWidth + horizontalGap) - horizontalGap + 150))}px}.fs-grid{justify-content:center}.fs-wide{grid-column:span 1}.fs-label{height:42px!important;display:flex;align-items:center;justify-content:center}.fs-label h2{position:static;margin:0;max-width:100%}.fs-label.fs-has-price{height:${smallText ? 54 : 64}px!important;display:block;padding:${smallText ? "8px 8px 20px" : "10px 8px 24px"}}.fs-label.fs-has-price h2{position:static;margin:0 auto;max-width:100%}.fs-label.fs-has-price .fs-price{display:flex}.fs-section{margin:0 0 24px;flex-wrap:nowrap;gap:18px;align-items:center;padding-bottom:22px;border-bottom:1px solid #7acbff44}.fs-section h1{font-size:28px}.fs-section:has(.fs-profile) h1{display:none}.fs-profile{margin:0;display:flex;align-items:center;gap:14px;min-width:0;flex:1}.fs-profile img,.fs-avatar-placeholder{width:60px;height:60px;border-radius:50%;object-fit:cover;flex:none;border:2px solid #bdeaff;box-shadow:0 0 0 5px #75ccff15}.fs-profile>div{min-width:0;max-width:100%;overflow:hidden}.fs-profile small{display:block;color:#8dcff4;font-size:9px;letter-spacing:2px;margin-bottom:5px}.fs-profile strong{display:block;font-family:"Fortnite Display",sans-serif;font-size:34px;white-space:nowrap;font-weight:400;line-height:1.1}.fs-report-date{font-size:21px;padding:8px 0 8px 20px;border-left:1px solid #7acbff44}.fs-report-date small{font-size:8px;letter-spacing:1px}.fs-report-footer{margin-top:18px}` : ""}
</style></head><body class="${useFeaturedMosaic ? "fs-mosaic" : "fs-dense"}"><header class="fs-section"><h1>${escapeHtml(labels?.title || "BACK FROM THE VAULT")}</h1>${labels?.profileName ? `<div class="fs-profile">${labels.profileAvatar ? `<img src="${escapeHtml(labels.profileAvatar)}" alt="" onerror="this.style.visibility=\'hidden\'" />` : `<span class="fs-avatar-placeholder">?</span>`}<div><small>SHOP ALERTS</small><strong class="fs-fit" data-min-size="14">${escapeHtml(`${labels.profileName.slice(0, 64)}’s Alerts`)}</strong></div></div>` : ""}${labels ? `<img alt="" aria-hidden="true" style="width:38px;height:38px;flex:0 0 38px;order:2;filter:brightness(0) invert(1);opacity:.9" src="data:image/svg+xml;base64,${fs.readFileSync(`${process.cwd()}/assets/tabler-bell.svg`).toString("base64")}" />` : ""}${!labels || (labels.pageCount || 1) > 1 ? `<div class="fs-report-date"><small>${escapeHtml(labels?.subtitle || "SHOP DATE · UTC")}</small>${escapeHtml(shopDateLabel)}${labels?.pageCount ? ` / ${labels.pageCount}` : ""}</div>` : ""}</header><main class="fs-grid">${tiles}</main><footer class="fs-report-footer">${escapeHtml(labels?.footer || `${items.length} RETURNING ITEMS · ${minimumDays}+ DAYS AWAY`)}${items.some(item => item.price !== undefined && item.priceIsCurrent !== undefined) ? " · * LATEST KNOWN PRICE · NOT HISTORICAL" : ""}${logoUrl ? `<img class="fs-bot-logo" src="${escapeHtml(logoUrl)}" alt="Creeper Bot" onerror="this.style.display=\'none\'" />` : ""}</footer><script>window.fitFortniteText=()=>{document.querySelectorAll(".fs-fit").forEach(node=>{const element=node;const minimum=Number(element.dataset.minSize||7);let size=parseFloat(getComputedStyle(element).fontSize);while(element.scrollWidth>element.clientWidth&&size>minimum){size=Math.max(minimum,size-.5);element.style.fontSize=size+"px"}})};document.fonts.ready.then(window.fitFortniteText);</script></body></html>`;
}
