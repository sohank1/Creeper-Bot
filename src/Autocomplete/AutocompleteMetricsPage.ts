...
function metricDay(value: unknown): string | null {
...
function autocompleteChartDays(snapshot: AutocompleteMetricsSnapshot, count = 30): string[] {
    const dates = snapshot.daily.map(point => metricDay(point.date)).filter(Boolean) as string[];
    if (!dates.length) return [];
    const latest = new Date(`${dates.sort().pop()}T00:00:00.000Z`);
    const result: string[] = [];
    for (let offset = count - 1; offset >= 0; offset--) {
        result.push(new Date(latest.getTime() - offset * 24 * 60 * 60 * 1000).toISOString().slice(0, 10));
    }
    return result;
}

function autocompleteDayLabel(day: string): string {
    return new Date(`${day}T00:00:00.000Z`).toLocaleDateString("en-US", { month: "short", day: "numeric", timeZone: "UTC" });
}

function dailyAutocompletePoint(snapshot: AutocompleteMetricsSnapshot, day: string): AutocompleteMetricsSnapshot["daily"][number] | undefined {
    return snapshot.daily.find(point => point.date === day);
}

function uniqueMetricUsernames(snapshot: AutocompleteMetricsSnapshot): number {
    const usernames = new Set<string>();
    for (const row of snapshot.rows) {
        const history = Array.isArray(row.usernameHistory) ? row.usernameHistory : [];
        for (const user of history) if (user.username) usernames.add(user.username);
        if (row.lastUsername) usernames.add(row.lastUsername);
    }
...
function renderMetricsPage(snapshot: AutocompleteMetricsSnapshot): string {
    ...
    const days = autocompleteChartDays(snapshot);
    const labels = days.map(autocompleteDayLabel);
    const topSurfaces = Array.from(snapshot.rows.reduce((counts, row) => {
        const surface = String(row.surface || "unknown");
        counts.set(surface, (counts.get(surface) || 0) + (Number(row.requests) || 0));
        return counts;
    }, new Map<string, number>()).entries()).sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0])).slice(0, 8);
    const charts = `<div class="chart-grid">
${renderMetricsChart({
        id: "autocomplete-volume",
        title: "Search volume over time",
        description: days.length ? `Daily autocomplete activity · last ${days.length} UTC days` : "Daily autocomplete activity",
        type: "line",
        labels,
        datasets: [
            { label: "Requests", data: days.map(day => Number(dailyAutocompletePoint(snapshot, day)?.requests || 0)), color: "blue" },
            { label: "Zero results", data: days.map(day => Number(dailyAutocompletePoint(snapshot, day)?.zeroResultRequests || 0)), color: "orange" },
            { label: "Errors", data: days.map(day => Number(dailyAutocompletePoint(snapshot, day)?.failedResponses || 0)), color: "red" }
        ],
        emptyMessage: "Daily history is not in the older metrics file yet. New searches will populate this graph."
    })}
${renderMetricsChart({
        id: "autocomplete-surfaces",
        title: "Searches by feature",
        description: "All-time requests grouped by autocomplete surface",
        type: "bar",
        labels: topSurfaces.map(([surface]) => surface),
        datasets: [{ label: "Requests", data: topSurfaces.map(([, requests]) => requests), color: "green" }],
        emptyMessage: "No autocomplete requests have been recorded yet."
    })}
</div>`;
    ...
    const status = snapshot.available
        ? `JSON metrics file is available. Showing all ${snapshot.rows.length} query keys for this namespace${snapshot.daily.length ? ` and ${snapshot.daily.length} daily rollup points` : ""}.`
        : `${snapshot.error || "Metrics are unavailable."} Autocomplete itself continues to work.`;
    const successRate = totals.requests ? ((Number(totals.successfulResponses) / Number(totals.requests)) * 100).toFixed(1) : "0.0";
    const zeroRate = totals.requests ? ((Number(totals.zeroResultRequests) / Number(totals.requests)) * 100).toFixed(1) : "0.0";
    const averageLatency = totals.requests ? Number(totals.durationTotalMs) / Number(totals.requests) : 0;
    const chartClient = charts.includes("metrics-chart-data") ? renderMetricsChartClient() : "";
    return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<meta http-equiv="refresh" content="60"><title>Creeper Bot · Autocomplete Metrics</title>
<style>
:root{color-scheme:dark;--bg:#08111f;--panel:#101d30;--line:#263a55;--text:#edf5ff;--muted:#94a8c2;--blue:#69d5ff;--green:#75e0ae;--orange:#ffc46b;--red:#ff8f8f;--purple:#c9a5ff}
...
<div class="card"><label>Success rate</label><strong>${successRate}%</strong><small>${formatNumber(totals.successfulResponses)} successful</small></div>
<div class="card"><label>Zero-result rate</label><strong>${zeroRate}%</strong><small>${formatNumber(totals.zeroResultRequests)} requests</small></div>
...
<div class="card"><label>Average latency</label><strong>${formatDuration(averageLatency)}</strong><small>all recorded requests</small></div>
<div class="card"><label>Users seen</label><strong>${formatNumber(uniqueMetricUsernames(snapshot))}</strong><small>full retained history</small></div>
<div class="card"><label>Pending query keys</label><strong>${formatNumber(snapshot.pendingKeys)}</strong><small>in memory before flush</small></div>
</section>
${charts}
...
</tbody></table></section></main>${chartClient}
